    ...
    <section className="relative overflow-hidden bg-brand">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-14 md:py-20 lg:grid-cols-2">
        ...
        >
          ...
          <ul className="mt-6 grid max-w-lg grid-cols-2 gap-x-4 gap-y-2.5 sm:grid-cols-3 stagger-children">
            {[
              ...
              <li
                key={f}
                className="flex items-center gap-2 text-sm text-brand-foreground/90 animate-fade-in-up"
              >
        ...
        >
          ...
          <div
            className="absolute -bottom-4 -left-4 hidden rounded-2xl bg-card p-4 shadow-xl animate-fade-in-up sm:block"
          ...
          <div
            className="absolute -right-3 top-6 hidden rounded-2xl bg-green px-4 py-3 text-green-foreground shadow-xl animate-fade-in-up sm:block"
            style={{ animationDelay: '700ms' }}
          >
            <p className="font-heading text-xl font-bold">৫০০০+</p>
            <p className="text-xs opacity-90">{t('successfulStudents')}</p>
          </div>
        </div>
      </div>
    </section>
