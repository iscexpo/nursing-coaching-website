...
export function ResultsTable({ examSubmissions }: { examSubmissions: ExamSubmission[] }) {
  return (
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {examSubmissions.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-b border-border last:border-0">
                  <td className="px-4 py-3 font-medium text-foreground">{r.examTitle || 'পরীক্ষা'}</td>
                  <td className="px-4 py-3 text-muted-foreground">{new Date(r.createdAt).toLocaleDateString('bn-BD')}</td>
            ...
            {examSubmissions.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-muted-foreground">কোনো ফলাফল নেই</td></tr>
            )}
          </tbody>
        </table>
      </div>
