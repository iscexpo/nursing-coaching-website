...
const smallInputCls =
  'mt-1 block w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand'
    ...
    fetch('/api/courses')
      ...
      .then((data) => {
        ...
        const activeCourses = list
          ...
          .map((c: { slug: string; title: string; fee: number }) => ({
            slug: c.slug,
            title: c.title,
            fee: c.fee,
          }))
        ...
        if (
          defaultCourse &&
          activeCourses.some(
            (course: CourseOption) => course.slug === defaultCourse,
          )
        ) {
    ...
    try {
      const res = await fetch('/api/admission', {
        ...
        body: JSON.stringify({
          ...
          honors:
            form.honors.roll || form.honors.registrationNo
              ? form.honors
              : undefined,
        }),
      })
                    ...
                    <div className="space-y-5 animate-fade-in">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mb-4">
                          আপনার SSC, HSC এবং অনার্স তথ্য প্রদান করুন।
                        </p>
                      </div>
                      <EduFields
                        label="SSC (মাধ্যমিক সার্টিফিকেট)"
                        value={form.ssc}
                        onChange={(ssc) => setForm({ ...form, ssc })}
                      />
                      <EduFields
                        label="HSC (উচ্চ মাধ্যমিক সার্টিফিকেট) (ঐচ্ছিক)"
                        value={form.hsc}
                        onChange={(hsc) => setForm({ ...form, hsc })}
                      />
                      <EduFields
                        label="অনার্স (স্নাতক) (ঐচ্ছিক)"
                        value={form.honors}
                    ...
                    <div className="space-y-5 animate-fade-in">
                      <div>
                        <p className="text-sm font-medium text-foreground">
                          আপনার আগ্রহী কোর্স নির্বাচন করুন *
                        </p>
                        <select
                          id="course"
                          value={form.course}
                      ...
                      <div className="rounded-2xl border border-border bg-secondary/30 p-4 text-sm text-muted-foreground">
                        ...
                        <p className="mt-2">
                          নির্বাচিত কোর্সের উপর ভিত্তি করে আমাদের টিম আপনার সাথে
                          যোগাযোগ করবে।
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="rounded-2xl border border-border bg-secondary/30 p-5">
                        ...
                        <dl className="mt-4 space-y-3 text-sm">
                          ...
                          <div className="flex justify-between gap-4 rounded-lg bg-background/50 px-3 py-2">
                            ...
                            <dd className="text-muted-foreground">
                              {coursesList.find((c) => c.slug === form.course)
                                ?.title || '-'}
                            </dd>
                          </div>
                          <div className="flex justify-between gap-4 rounded-lg bg-background/50 px-3 py-2">
                          ...
                          {form.ssc.roll && (
                            <div className="flex justify-between gap-4 rounded-lg bg-background/50 px-3 py-2">
                              ...
                              <dd className="text-muted-foreground text-right">
                                রোল: {form.ssc.roll}, রেজি:{' '}
                                {form.ssc.registrationNo}, বোর্ড:{' '}
                                {form.ssc.board || '-'}
                              </dd>
                            </div>
                          )}
                          {form.hsc.roll && (
                            <div className="flex justify-between gap-4 rounded-lg bg-background/50 px-3 py-2">
                              ...
                              <dd className="text-muted-foreground text-right">
                                রোল: {form.hsc.roll}, রেজি:{' '}
                                {form.hsc.registrationNo}, বোর্ড:{' '}
                                {form.hsc.board || '-'}
                              </dd>
                            </div>
                          )}
