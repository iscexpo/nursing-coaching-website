...
function SingleSmsSection() {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setStatus({
        ok: true,
        text: `পাঠানো হয়েছে — ${data.sent} জনকে (${data.provider})`,
      })
      if (!data.skipped) {
        setPhone('')
        setMessage('')
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
      ...
      {status && (
        <p
          className={`text-sm ${status.ok ? 'text-green-600' : 'text-destructive'}`}
        >
          {status.text}
        </p>
      )}
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h4 className="font-heading font-semibold text-foreground">
          বাল্ক SMS (সকল শিক্ষার্থী)
        </h4>
      </div>
      <p className="text-sm text-muted-foreground">
        সকল নিবন্ধিত শিক্ষার্থীদের SMS পাঠান
      </p>
      <div>
        <label className="block text-sm font-medium text-foreground">
          বার্তা
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h4 className="font-heading font-semibold text-foreground">
          CSV থেকে SMS (মার্কেটিং)
        </h4>
      </div>
      <p className="text-sm text-muted-foreground">
        CSV/Excel ফাইলে ফোন নম্বর দিয়ে SMS পাঠান
      </p>
      <div>
        <label className="block text-sm font-medium text-foreground">
          ফোন নম্বরের CSV/Excel ফাইল
        </label>
        <input
          ref={fileRef}
          type="file"
      ...
      <button
        onClick={handleSend}
        disabled={sending || !file || !message.trim()}
        className="flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-brand-foreground hover:bg-brand/90 disabled:opacity-50"
      >
        {sending ? (
          <Loader2 className="size-4 animate-spin" />
