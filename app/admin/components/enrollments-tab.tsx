...
export function EnrollmentsPanel({
  ...
  async function handleEditSave() {
    ...
    } catch { setEditError('আপডেট ব্যর্থ') }
  ...
  async function handleCancel(id: string) {
    ...
    try {
      ...
      if (res.ok) onRefresh()
      else { const err = await res.json().catch(() => ({})); alert(err.error || 'বাতিল ব্যর্থ') }
    } catch { alert('বাতিল ব্যর্থ') }
  ...
  return (
    <div className="space-y-4">
      ...
      {showAdd && (
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-heading font-semibold text-foreground">নতুন এনরোলমেন্ট</h4>
            <button onClick={() => setShowAdd(false)} className="text-muted-foreground hover:text-foreground"><X className="size-5" /></button>
          </div>
      ...
      {editing && (
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-heading font-semibold text-foreground">এনরোলমেন্ট সম্পাদনা — {editing.userName || '—'}</h4>
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <label className={labelCls}>নোট</label>
                <input type="text" value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} placeholder="নোট" className={inputCls} />
              </div>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
              ...
              <div>
                ...
                <input type="date" value={editForm.endDate} onChange={(e) => setEditForm({ ...editForm, endDate: e.target.value })} className={inputCls} />
              </div>
            </div>
            <div className="rounded-lg border border-border bg-secondary/30 p-3 space-y-2">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div>
                  <label className="block text-xs font-medium text-muted-foreground">কোর্স ফি</label>
                  <div className="mt-1 text-sm text-foreground">৳{editCourseFee.toLocaleString()}</div>
                </div>
                <div>
                  <label className={labelCls}>ছাড় (৳)</label>
                  <input type="number" min="0" max={editCourseFee} value={editForm.discount} onChange={(e) => setEditForm({ ...editForm, discount: e.target.value })} placeholder="০" className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted-foreground">পরিশোধযোগ্য ফি</label>
                  <div className="mt-1 text-sm font-semibold text-green">৳{editTotalFee.toLocaleString()}</div>
                </div>
              </div>
              <div className="text-xs text-muted-foreground">
                পরিশোধ: ৳{editing.paidAmount.toLocaleString()} | বকেয়: ৳{Math.max(0, editTotalFee - editing.paidAmount).toLocaleString()}
              </div>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="শিক্ষার্থী, ফোন বা কোর্স দিয়ে খুঁজুন..." value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-muted-foreground">
                  {search ? 'কোনো এনরোলমেন্ট পাওয়া যায়নি' : 'এখনো কোনো এনরোলমেন্ট নেই'}
                </td></tr>
              ) : filtered.map((e) => (
                <tr key={e.id} className="border-b border-border last:border-0 transition-colors hover:bg-secondary/50">
                  ...
                  <td className="px-4 py-3 text-center text-muted-foreground">
                    {e.discount > 0 ? <span className="text-orange-600 dark:text-orange-400">−৳{e.discount.toLocaleString()}</span> : '—'}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      {e.status !== 'cancelled' && (
                        <>
                          <button onClick={() => handleEditClick(e)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-secondary hover:text-foreground" title="সম্পাদনা">
                            <Pencil className="size-4" />
                          </button>
                          <button onClick={() => handleCancel(e.id)} disabled={cancelling === e.id}
                            className="rounded-lg p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" title="বাতিল করুন">
                            {cancelling === e.id ? <Loader2 className="size-4 animate-spin" /> : <Ban className="size-4" />}
                          </button>
                        </>
                      )}
                    </div>
