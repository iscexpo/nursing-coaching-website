...
import { Users, BookOpen, GraduationCap, Wallet, TrendingUp, Clock, CheckCircle2, AlertTriangle } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="সক্রিয় শিক্ষার্থী" value={totalStudents} sub="এনরোলড" icon={Users} color="brand" />
        <StatCard label="সক্রিয় কোর্স" value={activeCourses} sub={`মোট ${courses.length}টি`} icon={BookOpen} color="green" />
        <StatCard label="অপেক্ষমান" value={pendingEnrollments} sub="এনরোলমেন্ট" icon={Clock} color="gold" />
        <StatCard label="অপেক্ষমান পেমেন্ট" value={pendingPayments} sub="যাচাই বাকি" icon={AlertTriangle} color="red" />
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">মোট আয় (যাচাইকৃত পেমেন্ট)</p>
          </div>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {enrollments.slice(0, 5).map((e) => (
              <div key={e.id} className="flex items-center justify-between rounded-xl bg-secondary/30 px-3 py-2.5">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-foreground">{e.userName || 'শিক্ষার্থী'}</p>
                ...
                <span className={`ml-3 shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${
                  ...
                  e.status === 'pending' ? 'bg-gold/15 text-gold' :
                  e.status === 'approved' ? 'bg-brand/10 text-brand' :
                  'bg-secondary text-muted-foreground'
                }`}>
                  {e.status === 'active' ? 'সক্রিয়' : e.status === 'pending' ? 'অপেক্ষমান' : e.status === 'approved' ? 'অনুমোদিত' : e.status}
                </span>
              </div>
            ))}
            {enrollments.length === 0 && (
              <p className="py-4 text-center text-sm text-muted-foreground">কোনো এনরোলমেন্ট নেই</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {payments.filter((p) => p.status === 'pending').slice(0, 5).map((p) => (
              <div key={p.id} className="flex items-center justify-between rounded-xl bg-secondary/30 px-3 py-2.5">
                <div className="min-w-0 flex-1">
            ...
            {payments.filter((p) => p.status === 'pending').length === 0 && (
              <div className="flex flex-col items-center py-6">
                ...
                <p className="mt-2 text-sm text-muted-foreground">কোনো অপেক্ষমান পেমেন্ট নেই</p>
              </div>
            )}
          </div>
        </div>
