          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground">সময়কাল (মিনিট)</label>
                  <input type="number" value={examForm.duration} onChange={(e) => setExamForm({ ...examForm, duration: Number(e.target.value) })}
                    className="mt-1 block w-full rounded-lg border border-border bg-background px-3 py-2 text-foreground focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground">কঠিনতা</label>
                  <select value={examForm.difficulty} onChange={(e) => setExamForm({ ...examForm, difficulty: e.target.value as 'easy' | 'medium' | 'hard' })}
                    className="mt-1 block w-full rounded-lg border border-border bg-background px-3 py-2 text-foreground focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand">
              ...
              <button onClick={handleCreateExam} disabled={saving} className="flex items-center gap-1.5 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-brand-foreground hover:bg-brand/90 disabled:opacity-50">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  ...
                  <th className="px-4 py-3 text-center font-semibold text-foreground">সময়কাল</th>
                  <th className="px-4 py-3 text-center font-semibold text-foreground">প্রশ্ন</th>
                  <th className="px-4 py-3 text-center font-semibold text-foreground">কঠিনতা</th>
              ...
              <tbody>
                {exams.map((e) => (
                  <tr key={e.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 text-center text-muted-foreground flex items-center justify-center gap-1">
                      <Clock className="size-3.5" />{e.duration} মিনিট
                    </td>
                    <td className="px-4 py-3 text-center text-foreground">{e.questionCount ?? 0}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${DIFFICULTY_LABELS[e.difficulty]?.cls || ''}`}>
                        {DIFFICULTY_LABELS[e.difficulty]?.label || e.difficulty}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => toggleActive(e.id, e.isActive)} className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold cursor-pointer transition-colors ${e.isActive ? 'bg-green/10 text-green' : 'bg-secondary text-muted-foreground'}`}>
                ...
                {exams.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-muted-foreground">কোনো পরীক্ষা নেই</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  ...
                  <th className="px-4 py-3 text-center font-semibold text-foreground">সময়</th>
                ...
                {filteredSubmissions.map((s) => {
                  ...
                  return (
                    <tr key={s.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{new Date(s.createdAt).toLocaleDateString('bn-BD')}</td>
                    </tr>
                  )
                })}
                {filteredSubmissions.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-muted-foreground">কোনো ফলাফল নেই</td></tr>
                )}
              </tbody>
            </table>
          </div>
