  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (existing.status !== 'pending') {
        const err = new Error('Payment has already been processed') as Error & {
          status?: number
        }
        err.status = 400
        throw err
      ...
      if (status === 'verified') {
        if (!enrollment) {
          const err = new Error('Enrollment not found for payment') as Error & {
            status?: number
          }
          err.status = 400
          throw err
        ...
        if (!paymentCheck.ok) {
          const err = new Error(paymentCheck.error) as Error & {
            status?: number
          }
          err.status = 400
          throw err
      ...
      if (!updated) {
        const err = new Error('Conflict: payment status changed') as Error & {
          status?: number
        }
        err.status = 409
        throw err
