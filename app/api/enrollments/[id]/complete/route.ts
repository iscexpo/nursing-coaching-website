...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.transaction(async (tx) => {
    const [updated] = await tx.update(enrollments).set({ status: 'completed', completedAt: now, completionPercentage: 100, updatedAt: now }).where(and(eq(enrollments.id, id), eq(enrollments.status, existing.status))).returning()
    if (!updated) return null
    await tx.insert(studentLifecycleEvents).values({ id: randomUUID(), studentId: existing.userId, enrollmentId: id, eventType: 'enrollment.completed', details: { previousStatus: existing.status } })
    return updated
  })
  if (!result) return NextResponse.json({ error: 'Conflict: enrollment status changed' }, { status: 409 })
  const session = await getSession()
  try {
    await writeAudit(buildAuditEntry({ resourceType: 'enrollment', resourceId: id, action: 'complete' }, session, request.headers.get('x-forwarded-for') ?? undefined))
  } catch (auditError) {
    console.error('Failed to write audit log for enrollment complete', auditError)
