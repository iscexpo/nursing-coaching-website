...
test.describe('Admin Panel - Extended Features', () => {
  ...
  test.describe('Question Bank Management', () => {
    ...
    test('should create new question', async ({ page }) => {
      ...
      if (options > 1) {
        ...
        await page.fill('textarea[placeholder*="প্রশ্ন লিখুন"]', questionData.question);
        ...
        await expect(page.locator('text=' + questionData.question.substring(0, 20))).toBeVisible();
      }
    });
    ...
    test('should edit existing question', async ({ page }) => {
      ...
      if (options > 1) {
        ...
        if (count > 0) {
          ...
          // Verify update
          await expect(page.locator('text=' + newQuestion.substring(0, 20))).toBeVisible();
        }
      }
    });
    ...
    test('should delete question', async ({ page }) => {
      ...
      if (options > 1) {
        ...
        if (initialRows > 0) {
          // Click delete button
          const deleteButtons = page.locator('button').filter({ hasText: '' }).nth(1);
  ...
  test.describe('Notifications', () => {
    ...
    test('should send notification', async ({ page }) => {
      ...
      await page.fill('textarea[placeholder*="বিবরণ"]', notificationData.message);
  ...
  test.describe('SMS Features', () => {
    ...
    test('should send single SMS', async ({ page }) => {
      ...
      await page.fill('textarea[placeholder*="SMS লিখুন"]', faker.lorem.sentence());
      ...
      await expect(page.locator('input[placeholder*="01XXXXXXXXX"]')).toHaveValue('01712345678');
    });
    ...
    test('should upload CSV for SMS', async ({ page }) => {
      ...
      await expect(page.locator('text=ফোন নম্বরের CSV/Excel ফাইল')).toBeVisible();
    });
  });
  ...
  test.describe('Media Management', () => {
    ...
    test('should show file type indicators', async ({ page }) => {
      ...
      if (count > 0) {
        ...
        await expect(page.locator('text=ALT').or(page.locator('text=DESC'))).toBeVisible();
      }
    });
  });
  ...
  test.describe('Reports and Analytics', () => {
    ...
    test('should export report to CSV', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("এক্সপোর্ট CSV")')).toBeVisible();
    });
  ...
  test.describe('Settings Configuration', () => {
    ...
    test('should update site name', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("সংরক্ষণ করুন")')).toBeVisible();
    });

    test('should update contact information', async ({ page }) => {
      await page.fill('input[placeholder*="01784-176442"]', '01712345678');
      await page.fill('input[placeholder*="info@iscexpo.edu.bd"]', 'test@example.com');
    ...
    test('should update social links', async ({ page }) => {
      await page.fill('input[placeholder*="https://wa.me"]', 'https://wa.me/8801234567890');
      await page.fill('input[placeholder*="https://www.facebook.com"]', 'https://facebook.com/test');
    ...
    test('should add Why ISC Expo item', async ({ page }) => {
      const initialCount = await page.locator('div.rounded-xl').filter({ hasText: 'কার্ড' }).count();
      
      await page.click('button:has-text("নতুন যোগ করুন")');
      
      const newCount = await page.locator('div.rounded-xl').filter({ hasText: 'কার্ড' }).count();
