...
/**
 * Compute new balances when a verified payment is refunded.
 * Paid amounts decrease; due amounts increase back up by the refund amount.
 */
export function calculatePaymentRefund(
  ...
  const newEnrollmentPaid = Math.max(0, enrollmentPaid - refundAmount)
  const newEnrollmentDue = enrollmentDue + refundAmount
  const newInvoicePaid = Math.max(0, invoicePaid - refundAmount)
  const newInvoiceDue = invoiceDue + refundAmount
