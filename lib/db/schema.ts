...
export const user = pgTable('user', {
  ...
  phoneNumberVerified: boolean('phone_number_verified')
    ...
    .default(false),
  role: text('role')
    .$type<'super-admin' | 'admin' | 'teacher' | 'student'>()
    .default('student')
    .notNull(),
...
export const courses = pgTable('courses', {
  ...
  category: text('category').$type<'icon' | 'isc'>().notNull().default('icon'),
...
export const invoices = pgTable(
  'invoices',
  {
    ...
    dueAmount: integer('due_amount').notNull(),
    status: text('status')
      .$type<'unpaid' | 'partial' | 'paid' | 'overdue'>()
      .default('unpaid')
      .notNull(),
...
export const settings = pgTable('settings', {
  ...
  siteName: text('site_name')
    .notNull()
    .default('ISC Expo - Icon Skill & Career Expo'),
  siteTagline: text('site_tagline')
    .notNull()
    .default('সাফল্যের জন্য প্রস্তুতি'),
...
export const subjects = pgTable(
  ...
  (table) => [index('subjects_is_active_idx').on(table.isActive)],
)
...
export const exams = pgTable(
  'exams',
  {
    ...
    difficulty: text('difficulty')
      ...
      .notNull(),
    isActive: boolean('is_active').notNull().default(true),
    examType: text('exam_type').default('model_test'),
...
export const admissions = pgTable(
  'admissions',
  {
    ...
    status: text('status')
      .$type<'pending' | 'received' | 'processing' | 'approved' | 'rejected'>()
      .notNull()
      .default('pending'),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow(),
...
export const admitCards = pgTable(
  'admit_cards',
  {
    id: text('id').primaryKey(),
    userId: text('user_id')
