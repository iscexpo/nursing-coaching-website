...
|---|------|-------|---------------------|
| 1.1 | Rebuild migration journal | `lib/db/migrations/meta/_journal.json` | All 15 migration entries (0000–0014) present with correct file hashes |
| 1.2 | Run `drizzle-kit push` or seed endpoint to apply migrations | `api/admin/migrate` | DB tables match `lib/db/schema.ts` |
| 1.3 | Verify email+password sign-in | `lib/auth.ts`, `app/api/auth/[...all]/route.ts` | POST returns 200, session cookie set |
| 1.4 | Verify middleware auth guard | `middleware.ts` | Unauthenticated → `/auth/sign-in`; authenticated → `/admin` or `/dashboard` |
| 1.5 | Verify phone+OTP flow | `lib/auth.ts` (phone plugin), Supabase Edge Functions | OTP sent, verified, session created |
| 1.6 | Run existing unit tests (`pnpm test`) | `tests/*` | All 8 test files pass |
| 1.7 | Run typecheck (`pnpm typecheck`) | — | Zero errors |
| 1.8 | Run lint (`pnpm lint`) | — | Zero errors |
...
| 1.10 | Seed demo admin user | `scripts/seed-demo-admin.ts` | `admin@cornia.co` can log in and see all 22 tabs |
...
| 2C.10 | **Reports** | Date range picker, charts via `recharts` (revenue line, enrollment pie, attendance heatmap), PDF export. |
...
## Phase 3 — LMS Core Logic (Weeks 3–4)

### 3A: Enrollment Lifecycle
...
| 3E.5 | SMS integration for notifications | `lib/sms.ts`, notification API | Send SMS when template channel is `sms` |
...
| 4.2 | `ChartCard` component | `components/ui/chart-card.tsx` | Wraps recharts with title, loading, empty states |
...
| 4.9 | PDF export | `app/api/reports/export/[type]/route.ts` | Generate PDF using `@react-pdf/renderer` or `jspdf` |
| 4.10 | Reports tab UI | `app/admin/components/reports-tab.tsx` | Charts, date picker, export buttons per report type |
...
| # | Task | Files | Details |
|---|------|-------|---------|
| 5A.1 | CSRF token middleware | `middleware.ts` or new `lib/csrf.ts` | Generate/validate tokens for POST/PUT/DELETE |
| 5A.2 | Session hardening | `lib/auth.ts` | Invalidate on password change, max 3 concurrent sessions |
| 5A.3 | Session activity tracking | `lib/db/schema.ts` (migration) | `ip_address`, `user_agent`, `last_active_at` on session |
| 5A.4 | Force logout | `app/api/admin/logout-all/route.ts` | Delete all sessions for a user |
| 5A.5 | Audit log for file ops | `lib/audit.ts` | Log upload/delete with actor + file details |
| 5A.6 | Signed URLs for uploads | `lib/storage.ts` | Use Vercel Blob signed URLs with expiry |

### 5B: Testing

| # | Task | Target | Target Count |
|---|------|--------|--------------|
...
### Migration 0015: Enrollment Lifecycle
```sql
...
### Migration 0016: Exam Enhancements
```sql
...
### Migration 0017: Notifications & Session
```sql
...
### Migration 0018: Attendance & Leave
```sql
...
*Detailed design specifications: `docs/lms-admin-improvement-design.md`*
*QA review report: `docs/qa-review-report.md`*
